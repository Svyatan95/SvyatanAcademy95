...
{
    [Info("UberTool", "FuJiCuRa", "1.4.6", ResourceId = 78)]
    ...
    internal class UberTool : RustPlugin
    {
        ...
        public class EPlanner : MonoBehaviour
        {
                                ...
                                if (!isWiring)
                                {
                                    if (ioEntity != null)
                                    {
                                        ...
                                        for (int i = 0; i < slots.Length; i++)
                                        {
                                            ...
                                            {
                                                float d = Vector3.Distance(raycastHit.point, ioEntity.transform.TransformPoint(slot.handlePosition));
                                                if (d < distance)
                                                {
                                                    distance = d;
                                                    target = slot;
                    ...
                    else if (!ctvTrgt)
                    {
                        ...
                        else if (serverInput.WasJustPressed(controlButtons[CmdType.HammerToggleOnOff]) && rayEntity && rayEntity.IsValid() && !(rayEntity is BuildingBlock))
                        {
                            ...
                            if (r is StorageContainer || r is IOEntity)
                            {
                                ...
                                if (r is SearchLight && !isOn) (r as SearchLight).secondsRemaining = float.PositiveInfinity;
                        ...
                        {
                            string infoMsg = "";
                            if (Instance.enableHammerTCInfo && rayEntity && rayEntity is BuildingPrivlidge && rayEntity.Distance(player.transform.position) > 1.25f)
                            {
                                bool hasClans = Instance.Clans != null ? true : false;
                                StringBuilder sb = new StringBuilder();
                                rayEntityName = (rayEntity as BaseCombatEntity).repair.itemTarget?.displayName?.english;
                                sb.Append(
								$">\nAuthentication list of <color=#ffa500>{rayEntityName}</color> (<color=#00ffff>{rayEntity.net.ID}</color>)");
                            ...
                            if (Instance.enableHammerCodelockInfo && rayEntity && rayEntity.HasSlot(BaseEntity.Slot.Lock) && rayEntity.GetSlot(BaseEntity.Slot.Lock) is CodeLock)
                            {
                                ...
                                rayEntityName = (rayEntity as BaseCombatEntity).repair.itemTarget?.displayName.english;
                                sb.Append(
								$">\nCodeLock info for <color=#ffa500>{rayEntityName}</color> (<color=#00ffff>{rayEntity.net.ID}</color>)");
                                IPlayer iPlayer = Instance.covalence.Players.FindPlayerById(rayEntity.OwnerID.ToString());
                                if (iPlayer != null)
                                {
                                    sb.Append(
            ...
            private void DoPlacement()
            {
                ...
                if (gameObject != null)
                {
                    ...
                    if (dplybl != null)
                    {
                        ...
                        if (!(bsntt is MiningQuarry) && target.entity != null && !(target.entity is BuildingBlock))
                        {
            ...
            private void BldMnUI(float factor)
            {
                ...
                Vector2 mS = new Vector2(0.3425f, 0.5f);
                for (int i = 0; i < 15; i++)
                {
                    float theta = i * 2 * (float)PI / 15;
                    Vector2 myNext = RttByRdns(mC, mS, theta, factor);
                    element.Add(BuildIconUI(mainName, myNext, r("nffrgf/vpbaf/pvepyr_tenqvrag.cat"), -0.040f, 0.040f, "1 1 1 1", factor, false));
                    element.Add(BuildRawIconUI(mainName, myNext, Instance.prefabIdToImage[Instance.constructionIDs[i]], -0.02f, 0.02f, "0.2 0.5 0.8 0.5", factor, true));
                    element.Add(BuildButtonUI(mainName, Vector2.MoveTowards(myNext, mC, 0.06f), i, -0.020f, 0.020f, color, factor), mainName);
                    element.Add(BuildButtonUI(mainName, Vector2.MoveTowards(myNext, mC, 0.03f), i, -0.025f, 0.025f, color, factor), mainName);
                    element.Add(BuildButtonUI(mainName, myNext, i, -0.030f, 0.030f, color, factor), mainName);
                    element.Add(BuildButtonUI(mainName, Vector2.MoveTowards(myNext, mC, -0.02f), i, -0.035f, 0.035f, color, factor), mainName);
                }

                element.Add(CustomIconUI(mainName, new Vector2(0.85f, 0.5f), r("nffrgf/vpbaf/rkvg.cat"), -0.025f, 0.025f, "1 1 1 1", factor));
                element.Add(
                CustomButtonUI(mainName, new Vector2(0.85f, 0.5f), "ut.prefab 6666", -0.025f, 0.025f, color, factor), mainName);
                CuiHelper.AddUi(player, element);
            }

            private void DoCrosshair(string cColor =
        default(string), bool kill = false)
            {
            ...
            private void DoWarning(string cColor =
        default(string), bool kill = false)
            {
        ...
        private string[] cnstrctnNms = new string[] {
            "wall.third",
            "stairs_l",
            "stairs_u",
            "roof",
        ...
        private uint[] constructionIDs = new uint[] {
            310235277,
            3250880722,
            1961464529,
            3895720527,
        ...
        private void Loaded()
        {
            ...
            bool>();
            activeUberObjects = new Dictionary<ulong,
            EPlanner>();
        ...
        private void OnServerInitialized()
        {
            ...
            for (int i = 0; i < cnstrctnNms.Length; i++)
            {
                string icon = cnstrctnNms[i];
        ...
        private void GetIconIds()
        {
            for (int i = 0; i < cnstrctnNms.Length; i++)
            {
                prefabIdToImage.Add(constructionIDs[i], (string)ImageLibrary?.Call("GetImage", cnstrctnNms[i]));
            }
        ...
        private void OnPlayerTick(BasePlayer p, PlayerTick msg, bool wasPlayerStalled)
        {
            ...
            if (p && ctvUbrTls.TryGetValue(p.userID, out exists) && exists)
            {
                ...
                {
                    Instance.ctvUbrTls[p.userID] = false;
                    Instance.activeUberObjects[p.userID].OnDestroy();
                    Instance.activeUberObjects.Remove(p.userID);
                    return;
                }

                if (p.IsSleeping() || p.IsReceivingSnapshot || p.IsSpectating()) return;
                Instance.activeUberObjects[p.userID].SetHeldItem(msg.activeItem);
        ...
        private void OnStructureRepair(BaseCombatEntity bsntt, BasePlayer player)
        {
            ...
            if (player && ctvUbrTls.TryGetValue(player.userID, out exists) && exists)
            {
                if (enablePerimeterRepair)
                {
                    ...
                    Vis.Entities<BaseCombatEntity>(bsntt.transform.position, perimeterRepairRange, list, 1048704 << 1);
                    int repaired = 0;
        ...
        {
            Vector2 v = A - c;
            float x = v.x * Mathf.Cos(a) + v.y * Mathf.Sin(a);
            float y = (v.y * Mathf.Cos(a) - v.x * Mathf.Sin(a)) * f;
            Vector2 B = new Vector2(x, y) + c;
            return B;
        }

        private static string GeAn(Vector2 m, float s, float f)
        {
        ...
        private static CuiButton BuildButtonUI(string panelName, Vector2 p, int ct, float mi, float ma, string c, float f)
        {
            return new CuiButton
            {
                ...
                RectTransform = {
                    AnchorMin = GeAn(p, mi, f),
                    AnchorMax = GeAn(p, ma, f)
                },
                Text = {
                    Text = null
                }
        ...
        private static CuiElement BuildIconUI(string pN, Vector2 p, string iN, float mi, float ma, string c, float f, bool b)
        {
        ...
        private static CuiElement BuildRawIconUI(string pN, Vector2 p, string iN, float mi, float ma, string c, float f, bool b)
        {
            return new CuiElement
            {
                Parent = pN,
